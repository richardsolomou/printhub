...
test('manages a fair print queue and assigns work to printers', async ({ page }) => {
  ...
  await choose(page.getByLabel('Adapter'), 'S3-compatible or own server')
  await choose(page.getByLabel('Provider'), 'Own server, NAS, or custom S3')
  await expect(page.getByText('Using storage on your own machine?')).toBeVisible()
  await expect(page.getByLabel('S3 endpoint')).toHaveAttribute('placeholder', 'https://storage.example.com')
  await expect(page.getByLabel('Region')).toHaveValue('us-east-1')
  await screenshot(page, 'user-owned-storage')
