...
2. Keys are kebab-case strings (`printers`, `board`); values are typed JSON via `context.repository.getSetting<T>(key)` / `setSetting(key, value)`. Deployment settings go through the `deploymentSettings` accessor in `src/server/app.ts`.
3. Server function: zod schema in `src/server/schemas.ts`, then the standard fn shape (see the `adding-server-functions` skill) — `workspaceAdmin` guard for workspace settings, `superAdmin` for global settings.
