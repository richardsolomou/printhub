...
describe('tus upload transport', () => {
  ...
  it('creates, completes, and safely resumes an authenticated upload', async () => {
    ...
    const invalidThreeMf = Buffer.from('not a 3mf archive')
    const rejected = await handleUpload(
      new Request('http://print.test/api/upload', {
        method: 'POST',
        headers: {
          ...headers,
          'upload-length': String(invalidThreeMf.length),
          'upload-metadata': metadata({
            filename: 'broken.3mf',
            name: 'Broken',
            quantity: '1',
    ...
    expect(rejectedLocation).toBeTruthy()

    const failed = await handleUpload(
      new Request(`http://print.test${rejectedLocation}`, {
        method: 'PATCH',
        headers: { ...headers, 'content-type': 'application/offset+octet-stream', 'upload-offset': '0' },
        body: invalidThreeMf,
      }),
    ...
    expect(instance.repository.listRequests()).toHaveLength(1)
    const rejectedId = rejectedLocation!.split('/').at(-1)!
    expect((await fs.promises.readdir(path.join(process.env.DATA_DIR, 'tus'))).some((name) => name.startsWith(rejectedId))).toBe(false)
