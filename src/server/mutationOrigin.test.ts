...
describe('cookie-auth mutation origin guard', () => {
  afterEach(() => { delete process.env.AUTH_PROVIDER })
  ...
  it('leaves trusted-header mutations to proxy-secret authentication', () => {
    process.env.AUTH_PROVIDER = 'trusted-header'
    expect(() => requireMutationOrigin(new Request('https://print.test/_server'))).not.toThrow()
  })
})
