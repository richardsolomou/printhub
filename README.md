...
`DATA_DIR` (default `/data`) selects app data, `PRINTS_DIR` (default `/prints`) selects default local storage, `ASSET_JOB_CONCURRENCY` (default and maximum `8`) limits simultaneous thumbnail, preview, and resin-orientation jobs, and `METRICS_TOKEN` optionally protects Prometheus metrics. On TrueNAS, start with `ASSET_JOB_CONCURRENCY=2` for an HDD-backed pool or `4` for SSD storage; higher concurrency can make bulk backfills slower by turning sequential reads into competing random I/O. Product settings — storage backend, board visibility, telemetry, authentication, and email delivery — live in the admin-only **Settings** page and persist in `/data/printhub.sqlite`. Integration secrets are encrypted with `/data/integration-secrets.key` unless `INTEGRATIONS_ENCRYPTION_KEY` is supplied.
